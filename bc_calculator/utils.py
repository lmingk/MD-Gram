...
def barycentric_mapping2(Xt, coupling):

    transp_Xs = coupling / torch.sum(coupling, 1)[:, None]
    transp_Xs[~torch.isfinite(transp_Xs)] = 0
    Xt= torch.FloatTensor(Xt).to(transp_Xs)
    transp_Xs = torch.matmul(transp_Xs, Xt)
