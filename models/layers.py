...
class MetaGINConv3(MessagePassing):

    def __init__(self, input_size, output_size, bias = True, eps = 0., train_eps = True,cached=False, improved = False,**kwargs):
        kwargs.setdefault('aggr', 'add')
        super().__init__(**kwargs)
        self.input_size =input_size
        self.output_size = output_size
        self.weight = Parameter(torch.Tensor(input_size, output_size))
        if bias:
    ...
    def forward(self, x, edge_index,weight, bias,eps = 0,edge_weight=None):
        """"""
        ...
        out = self.propagate(edge_index, norm = norm,x=x)

        x_r = x[1]
        
        if x_r is not None:
            if self.train_eps:out += (1 + eps) * x_r
            else: out += (1 + self.eps) * x_r


        return torch.matmul(out, weight)+bias
...
class MetaGINConv2(MessagePassing):
    ...
    def __init__(self,
                 ...
                 train_eps = True,
                 **kwargs):
        super(MetaGINConv2, self).__init__(aggr='add', **kwargs)
    ...
    def forward(self, x, edge_index, weight, bias, eps = 0,edge_weight=None):
        ...
        edge_index, norm = self.cached_result
        if self.train_eps:
            out = (1 + eps)* x +  self.propagate(edge_index, x=x, norm=norm,)
        else:
            out = (1 + self.eps)* x +  self.propagate(edge_index, x=x, norm=norm)
...
class GIN_mlp(nn.Module):
    ...
    def forward(self,x,fc1_weight,fc1_bias,fc2_weight,fc2_bias):
...
class MetaGINConv(MessagePassing):
    ...
    def __init__(self, in_channels,out_channels, eps: float = 0., train_eps: bool = False,
        ...
        self.in_channels = in_channels
        self.out_channels=out_channels
        self.nn = GIN_mlp(in_channels,out_channels)
        self.train_eps = train_eps
        self.initial_eps = eps
        if train_eps:
            self.eps = torch.nn.Parameter(torch.Tensor([eps]))
    ...
    def forward(self, x, edge_index,fc1_weight,fc1_bias,fc2_weight,fc2_bias,eps=0,size = None) -> Tensor:
        """"""
        if isinstance(x, Tensor):
            x: OptPairTensor = (x, x)
        ...
        return self.nn(out,fc1_weight,fc1_bias,fc2_weight,fc2_bias)
