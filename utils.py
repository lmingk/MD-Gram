...
def test_mvg(model, x_text, x_topo, train_pos_edge_index, pos_edge_index, neg_edge_index,weights):
    model.eval()
    with torch.no_grad():
        z = model.encode(x_text, x_topo, train_pos_edge_index,weights)
    #print(z)
    return model.test(z, pos_edge_index, neg_edge_index)

def test_proto(model, x, train_pos_edge_index, pos_edge_index, neg_edge_index,weights,proto):
    model.eval()
    with torch.no_grad():
        model.training = False 
        z = model.encode(x, train_pos_edge_index,weights)
    return model.test_proto(z, pos_edge_index, neg_edge_index,proto)
...
def get_topo_embedding(edge_index,dims,length):

    degrees = np.ones(length,dtype=np.float64)
    for i in range(edge_index.shape[1]):
        degrees[edge_index[0,i]]+=1
        degrees[edge_index[1,i]]+=1
    position = torch.Tensor(degrees).unsqueeze(1).float()
    topo_x = torch.zeros(length,dims,requires_grad=False)
    exp_term = torch.exp(torch.arange(0, dims, 2).float() * -(math.log(10000.0) / dims))
    topo_x[:, 0::2] = torch.sin(position * exp_term) # take the odd (jump by 2)
    topo_x[:, 1::2] = torch.cos(position * exp_term) # take the even (jump by 2)
...
def get_eigs(edge_index,alpha = 0.2):
    import scipy.sparse as sp
    from scipy.sparse.linalg import eigs
    
    length = torch.max(edge_index).item()+1
    edge_index = edge_index.numpy()
    row = edge_index[0]
    col = edge_index[1]
    data = np.ones_like(row,dtype=np.float64)
    mat = sp.coo_matrix((data,(row,col)),shape=(length,length))
    I = sp.eye(mat.shape[0],mat.shape[1])
    mat=mat+I
    rowsum = np.array(mat.sum(1)).flatten()
    d_inv = 1.0 / (np.maximum(1.0, rowsum))
    d_mat_inv = sp.diags(d_inv, 0)
    adj = d_mat_inv.dot(mat)
    
    
    difussion = (I-(1-alpha)*adj).tocsr()
    vals,_ = eigs(difussion,k = 1800,which = 'SM')
    return 1/np.sort(vals)
